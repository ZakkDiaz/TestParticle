...
namespace ParticleLib.Models
{
    public class Particle : IRectQuadStorable
    {
        ...
        {
            var tot = Math.Sqrt((Math.Pow(dimenisons[1].vel, 2) + Math.Pow(dimenisons[0].vel, 2)));


            //var max = dimenisons[0].vel > dimenisons[1].vel ? dimenisons[0].vel : dimenisons[1].vel;
            //var scale = Math.Abs(dimenisons[0].vel / 1f);
            //var angle = Math.Atan2(dimenisons[1].vel, dimenisons[0].vel);
            
            //var angleDiff = angle - fAngle - (45 * Math.PI / 180);
            //var dim1 = Math.Sin(angle) * max;
            //var dim2 = Math.Cos(angle)* max;

            //var nA = angle + (fAngle * forceMult);
            var cos = Math.Cos(fAngle);
            var sin = Math.Sin(fAngle);


            //var Tmag = Math.Sqrt(Math.Pow(sin, 2) + Math.Pow(cos, 2));

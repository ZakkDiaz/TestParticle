...
namespace OctreeEngine
{
    public static class Helpers
    {
        ...
        public static bool ContainsPoint(this OctreeCellCollection collection, Point3D point) => true;

        public static Point3D Center(this OctreeCellCollection collection) => (collection.From + ((collection.To - collection.From) / 2));
        //removed math.abs from center...
        public static ulong GetQuad(this OctreeCellCollection collection, Point3D pLoc) =>
                (ulong)(
                     (pLoc.X > collection.Center().X ? 0b1001 : 0b1000)
                    |
                    (pLoc.Y > collection.Center().Y ? 0b1010 : 0b1000)
                    |
                    (pLoc.Z > collection.Center().Z ? 0b1100 : 0b1000)
                );
    }
}
